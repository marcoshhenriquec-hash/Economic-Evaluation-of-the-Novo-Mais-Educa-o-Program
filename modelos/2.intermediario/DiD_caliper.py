# ======================================================
# matching_ideb_caliper_025.py
# Matching por trajetória IDEB 2009-2015 com caliper 0.25
...
output_ai_escolas = r"C:\Users\suporte1\Desktop\lai\matched_escolas_IDEB_AI_caliper025.csv"
output_af_escolas = r"C:\Users\suporte1\Desktop\lai\matched_escolas_IDEB_AF_caliper025.csv"

output_ai_painel = r"C:\Users\suporte1\Desktop\lai\matched_painel_IDEB_AI_caliper025.csv"
output_af_painel = r"C:\Users\suporte1\Desktop\lai\matched_painel_IDEB_AF_caliper025.csv"

output_balanceamento = r"C:\Users\suporte1\Desktop\lai\balanceamento_matching_IDEB_caliper025.csv"

# Matching a partir de 2009
pre_years = [2009, 2011, 2013, 2015]

# Painel final mantém todos os anos disponíveis
anos_painel = [2005, 2007, 2009, 2011, 2013, 2015, 2017, 2019]

# Matching 1 controle por tratada
n_vizinhos = 1

# Caliper na distância euclidiana das variáveis padronizadas
caliper = 0.25

# Com reposição: controle pode ser par de mais de uma tratada
com_reposicao = True
...
def preparar_base_wide(df, outcome, etapa):
    ...
    """
    d = df.copy()
    d = d[d["ANO"].isin(anos_painel)].copy()
    ...
    # Exige IDEB nos anos usados no matching e em 2019
    ...
    # Deltas apenas para diagnóstico de balanceamento
...
def calcular_smd(df, vars_balance, grupo_col="tratamento_2017"):
    ...
    for var in vars_balance:
        ...
        smd = np.nan if sd_pool == 0 or np.isnan(sd_pool) else (mt - mc) / sd_pool
...
def fazer_matching_ideb_caliper(wide, etapa):
    """
    Matching nearest neighbor 1:k usando IDEB prévio padronizado.
    Aplica caliper: só aceita par se distância <= caliper.
    """
    ...
    print("\n==============================")
    print(f"MATCHING POR IDEB COM CALIPER 0.25 - {etapa}")
    print("==============================")
    print("Base wide:", wide.shape)
    print("Tratadas elegíveis antes do caliper:", tratadas.shape[0])
    ...
    if df_pares.empty:
        raise ValueError(f"Nenhum par encontrado para {etapa} com caliper {caliper}.")
    ...
    print("\nPares aceitos pelo caliper:", df_pares.shape[0])
    print("Tratadas mantidas após caliper:", matched_tratadas.shape[0])
    print("Tratadas descartadas pelo caliper:", tratadas.shape[0] - matched_tratadas.shape[0])
    print("Controles únicos matched:", matched_controles.shape[0])

    print("\nDistâncias dos pares aceitos:")
    print("Média:", df_pares["distancia"].mean())
    print("Mediana:", df_pares["distancia"].median())
    print("P90:", df_pares["distancia"].quantile(0.90))
    print("P95:", df_pares["distancia"].quantile(0.95))
    print("Máxima:", df_pares["distancia"].max())

    vars_balance = match_vars + [
        "delta_2009_2015",
        "delta_2011_2015",
        "delta_2013_2015"
    ]
    ...
    bal_depois["momento"] = "depois_matching_caliper025"
...
def montar_painel_matched(df_painel, matched_escolas, outcome, etapa):
    ...
    painel_matched["matching"] = "ideb_2009_2015_caliper025"
...
for etapa, outcome, output_escolas, output_painel in configs:
    ...
    matched, pares, balanceamento = fazer_matching_ideb_caliper(wide, etapa)
